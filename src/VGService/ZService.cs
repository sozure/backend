...
public class ZService
{
    ...
    private readonly Dictionary<string, Dictionary<string, string>> newValues = new()
    {
        ...
        ["CustomerService"] = new Dictionary<string, string>()
        {
        ...
        ["InternalLog"] = new Dictionary<string, string>()
        {
        ...
        ["PresenceReport"] = new Dictionary<string, string>()
        {
    ...
    public async Task RunChange(string environment)
    {
        ...
        foreach (var secret in filteredSecrets)
        {
            ...
            if (newValues.ContainsKey(msName) || msName == "Identityserver")
            {
                ...
                Dictionary<string, string> passwords = newValues[msName == "Identityserver"? "Identity" : msName];
                string password = passwords[environment];
                string newConnectionString;
                newConnectionString = $"Data Source=mssql-{environment}.db,1433;Initial Catalog={msName};User id={msName.ToLower() + "_usr"};Password={password}";
                Dictionary<string, string> newSecret = new Dictionary<string, string>()
                {
    ...
    public async Task CheckChange(string environment)
    {
        ...
        foreach(var secret in filteredSecrets)
        {
            ...
            if (newValues.ContainsKey(msName) || msName == "Identityserver")
            {
                Dictionary<string, string> passwords = newValues[msName == "Identityserver" ? "Identity" : msName];
                string password = passwords[environment];
                string goodConnString = $"Data Source=mssql-{environment}.db,1433;Initial Catalog={msName};User id={msName.ToLower() + "_usr"};Password={password}";
